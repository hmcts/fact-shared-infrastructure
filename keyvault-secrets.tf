...
locals {
  ...
  test_viewer_app_reg_name = "fact-frontend-${local.app_reg_suffix}"
}
...
data "azuread_application" "test_viewer_app_reg" {
  display_name = local.test_viewer_app_reg_name
}
...
resource "azurerm_key_vault_secret" "test_client_viewer_app_reg_id" {
  ...
  value        = data.azuread_application.test_viewer_app_reg.client_id
  key_vault_id = module.key_vault.key_vault_id
  tags = merge(var.common_tags, {
